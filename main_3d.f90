program main
  use mpi
  integer i,j,k,ierr, myid,numprocs,dims(3),sx,sy,sz,ex,ey,ez,nx,ny,nz,rc,comm3d,my_id,ntr
  REAL, DIMENSION(100):: DATA_
  real phir1,phir2,phir3
  logical periods(3)
 ...
 integer(kind=MPI_OFFSET_KIND), parameter :: zero_off = 0
  ...
  if (myid .eq. 0) then
     ...
     OPEN(UNIT=10,FILE='/home/linuxlover/CodingExperience/FORTRAN/DDC/OneDrive-2022-06-16/DDC3_mpi/pnew_3d.inp')
...
!     WRITE(*,*)   DTAU,TAUMAX,IFLAG,ARXY,CONMAS,NACELL
     ...
     WRITE(*,*)'  ITLIM,ITMASS,NTAU,NITMAX,NUSOPT,NTAUSTEP,NFUN,NKINE,INIT_ITER'
     READ(10,*)  
...
!     CALL GEOMET(X,Y,NX,NY,XTC,YTC)
     ...
     PHIR1 = ( 3.1415926/180)*DATA_(6)
     PHIR2 = ( 3.1415926/180)*DATA_(7)
     PHIR3 = ( 3.1415926/180)*DATA_(8)
     DATA_(6) = PHIR1;
     DATA_(7) = PHIR2;
     DATA_(8) = PHIR3;
  endif
...
!  call fnd2dnbrs( comm3d, nbreast, nbrwest, nbrnorth, nbrsouth, nbrtop, nbrbottom )
  ! Compute the decomposition
  !
  call fnd3ddecomp( comm3d, my_id, nx, ny, nz, sx, ex, sy, ey , sz, ez)
!  print *, 'my id = ', myid, my_id, sx,ex,sy,ey,sz,ez
  call mpi_3d(data_,sx,ex,sy,ey,sz,ez,nx,ny,nz,dims,zero_off,comm3d)
